...
- [1. 📝 题目描述](#1--题目描述)
- [2. 🎯 s.1 - 排序 + 二分查找 + 前缀最大值](#2--s1---排序--二分查找--前缀最大值)
...
- 时间复杂度：$O(n \log n)$，其中 n 是事件数组的长度
- 空间复杂度：$O(n)$，用于存储前缀最大值数组

算法思路：

- 按结束时间排序，维护前缀最大价值数组 maxVal
- 枚举每个事件，二分查找结束时间严格小于当前事件开始时间的最后一个事件
- 当前事件价值加上二分找到的前缀最大价值，更新全局最大值
