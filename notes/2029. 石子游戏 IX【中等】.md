---
id: 714be332-c7b6-4606-afba-449e0815f4b2
---
...
- 时间复杂度：$O(n)$，其中 n 是石子数量
- 空间复杂度：$O(1)$，只使用常数额外空间

算法思路：

- 将石子按模 3 的余数分为三类 `cnt[0]`、`cnt[1]`、`cnt[2]`
- 若 `cnt[0]` 为偶数，Alice 先手赢的条件是 `cnt[1] >= 1` 且 `cnt[2] >= 1`
- 若 `cnt[0]` 为奇数，Alice 先手赢的条件是 `|cnt[1] - cnt[2]| > 2`
