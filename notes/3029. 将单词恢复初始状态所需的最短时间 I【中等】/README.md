...
- 时间复杂度：$O(n^2 / k)$，最多枚举 n/k 次，每次比较 O(n) 字符
- 空间复杂度：$O(1)$，只使用常数额外空间

算法思路：

- 第 t 步后，前 t*k 个字符被替换，剩余后缀 word[t*k:] 需要等于原始 word 的前缀 word[0:n-t*k]
- 从 t=1 开始枚举，找到第一个满足后缀等于前缀的 t
- 如果没有匹配，则最大步数为 ceil(n/k)（全部替换）