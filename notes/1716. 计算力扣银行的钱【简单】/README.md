...
解释：
第 20 天后，总额为 (1 + 2 + 3 + 4 + 5 + 6 + 7) + (2 + 3 + 4 + 5 + 6 + 7 + 8) + (3 + 4 + 5 + 6 + 7 + 8) = 96。
```
...
- 计算完整周的存款总和：
  ...
  - 所有完整周的和为等差数列：$\sum_{i=0}^{weeks-1}(28 + 7i) = \frac{7 \cdot weeks \cdot (weeks + 7)}{2}$
- 计算剩余天数的存款：
  - 剩余天数从 $(1 + weeks)$ 开始的等差数列，共 rem 项
  - 和为：$\frac{rem \cdot (2 \cdot weeks + rem + 1)}{2}$
- 返回两部分之和
