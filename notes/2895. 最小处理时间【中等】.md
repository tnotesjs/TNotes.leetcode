---
id: ba50a5ab-c6ba-4fef-ad42-9c2bd419c5a0
---
...
- 时间复杂度：$O(n \log n)$，排序处理器和任务数组
- 空间复杂度：$O(1)$，排序使用原地操作

算法思路：

- 将处理器按空闲时间升序排序，任务按执行时间降序排序
- 贪心：最早空闲的处理器分配最耗时的 $4$ 个任务，以均衡最大完成时间
- 答案为所有处理器完成时间的最大值，即 $\max(processorTime[i] + tasks[4i])$
