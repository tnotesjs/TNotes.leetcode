...
解释：
将 s 中的单词按照初始位置排序，得到 "This1 is2 a3 sentence4"，然后删除数字。
```
...
解释：
将 s 中的单词按照初始位置排序，得到 "Me1 Myself2 and3 I4"，然后删除数字。
```
...
- 时间复杂度：$O(N)$，其中 N 是字符串 s 的长度，需要分割、遍历和拼接操作
- 空间复杂度：$O(N)$，需要存储分割后的单词数组和结果数组

算法思路：

- 按空格分割字符串得到单词数组 words
- 创建与 words 等长的数组 ordered 用于存放排序后的单词
- 遍历 words，提取每个单词末尾的数字索引（'1'→0, '2'→1, ...）
- 将去掉末尾数字的单词放入 ordered 的对应位置
- 用空格拼接 ordered 数组得到最终结果
