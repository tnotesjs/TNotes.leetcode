---
id: 63b55b1b-4fa1-43ec-9d9d-2c1c1b7ab1b3
---

## 1. 📝 题目描述

- [leetcode](https://leetcode.cn/problems/reformat-phone-number/)

给你一个字符串形式的电话号码 `number`。`number` 由数字、空格 `' '`、和破折号 `'-'` 组成。

请你按下述方式重新格式化电话号码。

- 首先，删除所有的空格和破折号。
- 其次，将数组从左到右每 3 个一组分块，直到剩下 4 个或更少数字。剩下的数字将按下述规定再分块：
  - 2 个数字：单个含 2 个数字的块。
  - 3 个数字：单个含 3 个数字的块。
  - 4 个数字：两个分别含 2 个数字的块。

最后用破折号将这些块连接起来。注意，重新格式化过程中不应该生成仅含 1 个数字的块，并且最多生成两个含 2 个数字的块。

返回格式化后的电话号码。

---

示例 1：

```txt
输入：number = "1-23-45 6"
输出："123-456"
```

解释：

- 数字是 "123456"
- 步骤 1：共有超过 4 个数字，所以先取 3 个数字分为一组。
  - 第 1 个块是 "123"。
- 步骤 2：剩下 3 个数字，将它们放入单个含 3 个数字的块。
  - 第 2 个块是 "456"。
- 连接这些块后得到 "123-456"。

---

示例 2：

```txt
输入：number = "123 4-567"
输出："123-45-67"
```

解释：

- 数字是 "1234567".
- 步骤 1：共有超过 4 个数字，所以先取 3 个数字分为一组。
  - 第 1 个块是 "123"。
- 步骤 2：剩下 4 个数字，所以将它们分成两个含 2 个数字的块。这 2 块分别是 "45" 和 "67"。
- 连接这些块后得到 "123-45-67"。

---

示例 3：

```txt
输入：number = "123 4-5678"
输出："123-456-78"
```

解释：

- 数字是 "12345678"。
- 步骤 1：第 1 个块 "123"。
- 步骤 2：第 2 个块 "456"。
- 步骤 3：剩下 2 个数字，将它们放入单个含 2 个数字的块。
  - 第 3 个块是 "78"。
- 连接这些块后得到 "123-456-78"。

---

示例 4：

```txt
输入：number = "12"
输出："12"
```

---

示例 5：

```txt
输入：number = "--17-5 229 35-39475 "
输出："175-229-353-94-75"
```

---

提示：

- `2 <= number.length <= 100`
- `number` 由数字和字符 `'-'` 及 `' '` 组成。
- `number` 中至少含 2 个数字。

## 2. 🎯 s.1 - 清洗后分块

::: code-group

```c [c]
char* reformatNumber(char* number) {
    // 清洗：去除空格和短横线
    char digits[110];
    int d = 0;
    for (int i = 0; number[i]; i++) {
        if (number[i] != ' ' && number[i] != '-') {
            digits[d++] = number[i];
        }
    }
    digits[d] = '\0';

    // 分块后拼接，最多 d 个数字 + d/3 个短横线
    char* res = (char*)malloc(d + d / 3 + 2);
    int i = 0, j = 0;
    while (d - i > 4) {
        if (j > 0) res[j++] = '-';
        res[j++] = digits[i++];
        res[j++] = digits[i++];
        res[j++] = digits[i++];
    }
    int remain = d - i;
    if (remain == 4) {
        if (j > 0) res[j++] = '-';
        res[j++] = digits[i++];
        res[j++] = digits[i++];
        res[j++] = '-';
        res[j++] = digits[i++];
        res[j++] = digits[i++];
    } else {
        if (j > 0) res[j++] = '-';
        while (i < d) res[j++] = digits[i++];
    }
    res[j] = '\0';
    return res;
}
```

```js [js]
/**
 * @param {string} number
 * @return {string}
 */
var reformatNumber = function (number) {
  const digits = number.replace(/[ \-]/g, "")
  const res = []
  let i = 0
  while (digits.length - i > 4) {
    res.push(digits.slice(i, i + 3))
    i += 3
  }

  const remain = digits.length - i
  if (remain === 4) {
    res.push(digits.slice(i, i + 2))
    res.push(digits.slice(i + 2, i + 4))
  } else {
    res.push(digits.slice(i))
  }

  return res.join("-")
}
```

```py [py]
class Solution:
    def reformatNumber(self, number: str) -> str:
        digits = number.replace(" ", "").replace("-", "")
        res = []
        i = 0
        while len(digits) - i > 4:
            res.append(digits[i:i + 3])
            i += 3
        remain = len(digits) - i
        if remain == 4:
            res.append(digits[i:i + 2])
            res.append(digits[i + 2:i + 4])
        else:
            res.append(digits[i:])
        return "-".join(res)
```

:::

- 时间复杂度：$O(N)$，其中 N 是字符串 number 的长度
- 空间复杂度：$O(N)$，需要存储清洗后的数字字符串和结果数组

算法思路：

- 使用正则表达式移除所有空格和短横线，得到纯数字字符串 `digits`
- 创建结果数组 `res`，初始化指针 `i` 为 0
- 当剩余数字超过 4 个时，循环取 3 个数字为一块，加入结果数组
- 处理剩余数字：
  - 如果剩余 4 个数字，拆分为两个 2 位数块
  - 如果剩余 2 或 3 个数字，直接作为一块加入结果数组
- 用短横线连接所有块，返回格式化后的电话号码
