---
id: d7f78740-ade3-4466-bdbc-a737846a4792
---
...
- 时间复杂度：$O(m + n \times C)$，其中 m 是 pick 数组长度，C 是颜色数量（最多 11 种）
- 空间复杂度：$O(n \times C)$，存储每个玩家每种颜色球的数量

算法思路：

- 用二维数组 `cnt[i][j]` 记录玩家 i 获得颜色 j 的球的数量
- 遍历 pick 数组，统计每个玩家每种颜色的球数
- 对于玩家 i，若存在某种颜色的球数量 > i，则为胜利玩家
