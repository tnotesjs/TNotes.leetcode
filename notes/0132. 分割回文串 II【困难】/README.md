...
- 枚举每个位置作为回文中心，并分别做两次中心扩展：
  - `(i, i)` 处理奇数长度回文
  - `(i, i + 1)` 处理偶数长度回文
  - 只要在边界内，且 `left` 和 `right` 指向的字符相同，就向两端扩散
- 状态转移方程：$cuts[right] = \min(cuts[right], cuts[left - 1] + 1)$
  - 当扩展得到一个回文区间 `s[left..right]` 时
  - 如果 `left == 0`，说明前缀 `s[0..right]` 本身就是回文串，此时 `newCut = 0`
  - 如果 `left != 0`，说明 `s[0...left - 1]` 区间还没有扩散到，需要根据 `cuts[left - 1]`（这段区间的最小分割次数）计算本次扩散点的最小切割数，也就是需要在 `left - 1` 和 `left` 之间切一刀，此时 `newCut = cuts[left - 1] + 1`
