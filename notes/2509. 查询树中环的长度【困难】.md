---
id: 709f371a-59a2-42d0-ad64-90aa26ec0861
---
...
- 时间复杂度：$O(m \cdot n)$，其中 $m$ 是查询数，$n$ 是树的深度
- 空间复杂度：$O(m)$，结果数组

算法思路：

- 完全二叉树中，节点 $x$ 的父节点为 $x / 2$
- 对每个查询 $(a, b)$，同时将较大的节点向上跳至父节点，直到 $a = b$
- 每次跳跃计数 $+1$，最终计数就是环的长度（包含新加的边）
