...
解释：
句子中的单词是 ["leetcode", "exercises", "sound", "delightful"]。
...
- 首先检查字符串首尾字符是否相等，不相等直接返回 `false`
- 遍历字符串，当遇到空格时，检查空格前一个字符（前一个单词的末尾）是否等于空格后一个字符（后一个单词的开头）
- 如果所有检查都通过，则说明是回环句
