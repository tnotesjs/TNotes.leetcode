---
id: 84dcdefd-f209-4bf0-943f-c2e01c52af3d
---
...
- 时间复杂度：$O(M)$，其中 M 是 logs 的长度，需要遍历所有任务
- 空间复杂度：$O(1)$，只使用常数额外空间

算法思路：

- 第一个任务从时刻 0 开始，用时为 logs[0][1]
- 从第二个任务开始，每个任务的用时 = 当前结束时间 - 上一个任务结束时间
- 遍历所有任务，记录用时最长的任务对应的员工 id
- 如果用时相同，选择 id 更小的员工
