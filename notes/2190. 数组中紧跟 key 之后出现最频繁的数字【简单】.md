---
id: 7c07e472-da42-41dc-93ee-21910179f91d
---
...
解释：
对于 target = 100，在下标 1 和 4 处出现过 2 次，且都紧跟着 key。
...
解释：
对于 target = 2，在下标 1，2 和 3 处出现过 3 次，且都紧跟着 key。
...
- 时间复杂度：$O(N)$，线性扫描累积 key 后的目标频次
- 空间复杂度：$O(N)$，哈希表记录目标计数（值域 1e3，可视作常数）

算法思路：

- 单次遍历，遇到 `nums[i] == key` 时统计 `nums[i+1]` 频次，并维护当前最高频及对应 target，遍历结束返回该 target。
